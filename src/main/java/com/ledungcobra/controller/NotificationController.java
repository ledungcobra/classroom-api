...
public class NotificationController {
    ...
    public ResponseEntity<?> getNotifications(@RequestParam("StartAt") Integer startAt, @RequestParam("MaxResults") Integer maxResults, @RequestParam("SortColumn") String sortColumn, @RequestParam("CurrentUser") String currentUser) {
        ...
        return ok(CommonResponse.builder().status(EStatus.Success).result(EResult.Successful).message("Get notifications successfully").content(NotificationsResponse.builder().hasMore(startAt + maxResults < numberOfNotifications).total(numberOfNotifications).data(NotificationData.builder().amountUnseen(notifications.stream().filter(n -> n.getIsSeen() == 0).count()).notifications(notifications.stream().map(n -> ReflectionUtils.mapFrom(n, new NotificationResponse[0])).toList()).build()).build()).build());
    }
    ...
    public ResponseEntity<?> putMarkNotificationSeen(@PathVariable("id") Integer id) {
        ...
        if (notification == null) {
            return ok(CommonResponse.builder().content("").message("Not found notification by id").result(EResult.Error).status(EStatus.Error).build());
        }

        notification.setIsSeen(true);
        notificationService.save(notification, user.getUserName());
        return ok(CommonResponse.builder().status(EStatus.Success).result(EResult.Successful).content("").message("Mark seen notification successfully").build());
    }
